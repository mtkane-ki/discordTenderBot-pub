...
async function main(storeID) {
  ...
  const saleSub = subItems.filter(function (subItem) {
    if (subItem.description.toLowerCase().includes("fresh baked")) {
...
async function getStores(zipCode) {
  ...
  const trimmedStores = top5Stores.map(({KEY, NAME, ADDR}) => ({StoreNumber: KEY, StoreName: NAME, StoreAddress: ADDR}));
